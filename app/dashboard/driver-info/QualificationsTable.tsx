...
export function QualificationsOverview({ onSelectDriver }: { onSelectDriver: (name: string) => void }) {
  return (
    <div className="card overflow-hidden">
      <div className="p-4 border-b border-b-border-default">
        <h3 className="text-sm font-bold text-primary">資格・保険 一覧表</h3>
      </div>
          ...
          <thead>
            <tr className="border-b border-b-border-default">
              {['ドライバー', '免許', '車検', '自賠責', '任意保険', '貨物保険', 'リース'].map(h => (
                <th key={h} className="text-left text-[10px] py-2.5 px-3 whitespace-nowrap text-muted">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {PLAYERS.map(p => {
              ...
              return (
                <tr key={p.name} className="transition-colors border-b border-b-border-subtle">
                  <td className="py-2.5 px-3">
                    <button onClick={() => onSelectDriver(p.name)}
                      className="flex items-center gap-2 cursor-pointer hover:underline bg-none border-0 p-0">
                  ...
                  {cells.map((cell, ci) => {
                    if (!cell.date) {
                      return <td key={ci} className="py-2.5 px-3 text-xs text-disabled">-</td>;
                    }
                    ...
                    return (
                      <td key={ci} className="py-2.5 px-3">
                        <div className="flex flex-col gap-0.5">
                          <span className="text-[11px] num text-secondary">{cell.date}</span>
                          {sev !== 'ok' && (
