  ...
  return (
    <div className="card p-5">
      <div className="flex items-center gap-2.5 mb-4">
        ...
        <div>
          <h2 className="text-sm font-bold" style={{ color: 'var(--text-primary)' }}>運送業界ニュース</h2>
          <p className="text-xs" style={{ color: 'var(--text-muted)' }}>軽貨物・物流の最新動向</p>
        </div>
      </div>
        ...
        <div className="space-y-3">
          {[1, 2, 3].map(i => (
            <div key={i} className="animate-pulse">
              <div className="h-3 rounded" style={{ background: 'var(--bg-elevated)', width: '90%' }} />
              <div className="h-2 rounded mt-2" style={{ background: 'var(--bg-elevated)', width: '40%' }} />
            </div>
          ))}
        </div>
      ) : news.length === 0 ? (
          ...
          {news.map((item, i) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 mt-1.5">
                <span className="text-xs" style={{ color: 'var(--text-disabled)' }}>{item.source}</span>
                <span className="text-xs" style={{ color: 'var(--text-disabled)' }}>{timeAgo(item.pubDate)}</span>
              </div>
            </a>
          ))}
        </div>
